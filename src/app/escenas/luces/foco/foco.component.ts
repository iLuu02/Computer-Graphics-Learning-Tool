...
export class FocoComponent implements AfterViewInit {
  ...
  private crearLuzFoco(args: string[] | undefined): void {
    if (!args || args.length !== 8) {
      this.notificationService.mostrarMensaje('Parámetros incorrectos para crear luz foco');
      return;
    }
  
    if (this.lights.length >= 10) {
      this.notificationService.mostrarMensaje('No se pueden crear más de 10 luces foco');
      return;
    }
    ...
    const angulo = Number(args[3]) * (Math.PI / 180);
    const penumbra = Number(args[4]);
    const x = Number(args[5]);
    const y = Number(args[6]);
    const z = Number(args[7]);
  
    if (isNaN(intensidad) || isNaN(distancia) || isNaN(angulo) || isNaN(penumbra) || isNaN(x) || isNaN(y) || isNaN(z)) {
      this.notificationService.mostrarMensaje('Intensidad, distancia, ángulo, penumbra y posición deben ser números');
      return;
    }
  ...
  private actualizarLuz(args: string[] | undefined): void {
    ...
    const angulo = Number(args[4]) * (Math.PI / 180);
    const penumbra = Number(args[5]);

    if (isNaN(id) || isNaN(intensidad) || isNaN(distancia) || isNaN(angulo) || isNaN(penumbra)) {
      this.notificationService.mostrarMensaje('ID, intensidad, distancia, ángulo y penumbra deben ser números');
      return;
    }
    ...
    if (light) {
      ...
      light.light.angle = angulo;
      light.light.penumbra = penumbra;
